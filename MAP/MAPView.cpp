...
void CMAPView::OnInitialUpdate()
{
	...
	// 2. 실패 시 메시지 박스 출력 (기존 코드)
	if (FAILED(hResult)) {
		AfxMessageBox(_T("이미지 로드 실패! 프로젝트 폴더에 MAP.png가 있는지 확인하세요."));
	}

	// ▼▼▼ [여기부터 아래 코드를 복사해서 추가하세요] ▼▼▼

	// 3. 프로그램 사용법 안내 팝업 띄우기
	CString strHelpMsg =
		_T("=============== [ 프로그램 사용법 ] ===============\n\n")
		_T("1. 🔵 노드(점) 생성\n")
		_T("   - 조작: 빈 곳에 마우스 [오른쪽 버튼] 클릭\n")
		_T("   - 동작: 파란색 원(노드)이 생성됩니다.\n\n")
		_T("2. ➖ 엣지(선) 연결\n")
		_T("   - 조작: [Ctrl 키] + 점 2개를 순서대로 [왼쪽 클릭]\n")
		_T("   - 동작: 두 점 사이의 파란 선과 거리가 표시됩니다.\n\n")
		_T("3. 🚩 최단 경로 탐색\n")
		_T("   - 조작: [Alt 키] + 출발점, 도착점 순서대로 [왼쪽 클릭]\n")
		_T("   - 동작: 최단 경로가 빨간색 굵은 선으로 표시됩니다.");
